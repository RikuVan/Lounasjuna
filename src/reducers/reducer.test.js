...
describe('auth reducer', () => {
  it('Should have user is loading on ATTEMPT_LOGIN', () => {
    ...
    expect(newState).toEqual(Object.assign({}, initialState.auth, {status: 'AWAITING_AUTH_RESPONSE'}))
  })
...
describe('requests reducer', () => {
  ...
  it('Should return data at keyed object on COMPLETE_REQUEST', () => {
    ...
    const requestAction = requestActions.completeRequest('restaurants', response)
    const newState = requests({}, requestAction)
    expect(newState)
      .toEqual({restaurants: Object.assign(response, {error: undefined, loading: false, resource: 'restaurants'})})
  })
})

describe('notifications reducer', () => {
  it('Should have keyed notification on SEND_NOTIFICATION', () => {
    const newState = notifications({}, {type: 'SEND_NOTIFICATION', payload: {key: 'OOPS', message: 'oops'}})
    expect(newState).toEqual({OOPS: {message: 'oops', key: 'OOPS'}})
  })
  it('Should remove keyed notification on DISMISS_NOTIFICATION', () => {
    const newState = notifications({}, {type: 'DISMISS_NOTIFICATION', payload: {key: 'OOPS'}})
